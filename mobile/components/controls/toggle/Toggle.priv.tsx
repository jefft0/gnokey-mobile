...
export const TogglePriv: React.FC<TogglePrivProps> = ({ checked = false, onChange, styleColors, testID }) => {
  const circleLeftPositionAnimation = React.useRef(new Animated.Value(checked ? CIRCLE_RIGHT_POSITION : 0)).current
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        styles.button,
        {
          backgroundColor: styleColors[checked ? 'toggleBackgroundActive' : 'toggleBackgroundInactive'],
    ...
    >
      ...
      >
        ...
        >
          <Ionicons name="checkmark-outline" size={18} color="black" />
        </Animated.View>
      </Animated.View>
    </TouchableOpacity>
  )
